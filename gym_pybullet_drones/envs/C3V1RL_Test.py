...
from gym_pybullet_drones.envs.C3V1Base_Test import C3V1Base_Test
...
class C3V1RL_Test(C3V1Base_Test):
    ...
    def _preprocessAction(self, action):
        ...
        for k in range(self.NUM_DRONES):  # 第 k 架drones
            ...
            # target = self.safe_action(target, k)  # 在很多架时可以使用的碰撞约束
    ...
    def enforce_altitude_limits(self, pos, v_unit_vector):
        safe_penalty = 0
        if pos[0] < -(self.all_axis * 1.5) and v_unit_vector[0] < 0:  # 限制x轴位置
            v_unit_vector[0] = 0.2
            safe_penalty += 5
        elif pos[0] > (self.all_axis * 1.5) and v_unit_vector[0] > 0:
            v_unit_vector[0] = -0.2
            safe_penalty += 5
        if pos[1] < -(self.all_axis * 1.5) and v_unit_vector[1] < 0:  # 限制y轴位置
            v_unit_vector[1] = 0.2
            safe_penalty += 5
        elif pos[1] > (self.all_axis * 1.5) and v_unit_vector[1] > 0:
            v_unit_vector[1] = -0.2
            safe_penalty += 5
        if pos[2] < 0 and v_unit_vector[2] < 0:  # 限制y轴位置
            v_unit_vector[2] = 0.2
            safe_penalty += 5
        elif pos[2] > self.all_axis and v_unit_vector[2] > 0:
            v_unit_vector[2] = -0.2
            safe_penalty += 5
