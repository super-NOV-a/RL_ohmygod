...
def generate_non_overlapping_positions_numpy(scale=1.0):
    ...
    cell_size = 0.5     # 放缩模型，现实与模型之比为10:1，此处单元格长宽为5m
    ...
    for cell_coord in cell_coordinates:  # 在每个单元格内随机生成一个位置
        x_center = (cell_coord[0] + 0.5) * cell_size - scale + np.random.uniform(0., 0.1)
        y_center = (cell_coord[1] + 0.5) * cell_size - scale + np.random.uniform(0., 0.1)
        z = np.random.uniform(0., 1. * scale)  # 保持z范围不变
        positions.append((x_center, y_center, z))

    return positions


class C3V1Base_Test(gym.Env):
    def __init__(self,
        ...
        #### Constants #############################################
        self.level = comm_level     # 通信等级，默认为0，即通信内容是位置信息
        self.all_axis = int(all_axis/10)      # 真实无人机与仿真无人机大小10:1，映射到环境中,轴长映射为原本的1/10
        self.G = 9.8
        ...
        if initial_xyzs is None:
            ...
            # todo 修改此处代码 修改成可以指定目标位置
            self.TARGET_POS = np.array([0, 0.5, 0.1])
    ...
    def get_init(self):
        ...
        if self.need_target:
            ...
            if self.all_axis >= 3:  # 如果范围过大
                drone_pos = init_pos[:self.NUM_DRONES]
                weights = np.random.uniform(0, 1, 3)  # 防止出现负z值
                weighted_avg = np.average(drone_pos, axis=0, weights=weights)
                return drone_pos, init_pos[self.NUM_DRONES], weighted_avg
    ...
    def show_target(self):
        ...
        for i in range(len(self.DRONE_IDS)):    # 关闭无人机之间的碰撞检测，用于展示
            for j in range(i + 1, len(self.DRONE_IDS)):
                model_id_1 = self.DRONE_IDS[i]
                model_id_2 = self.DRONE_IDS[j]
                p.setCollisionFilterPair(model_id_1, model_id_2, -1, -1, enableCollision=False)
    ...
    def get_new_target_position(self, total_steps=12000):
        ...
        # 计算角度范围，从 0 到 π
        if self.all_axis >= 3:   # 较大的场景，总步数会达到 更多12000*5, 总步数需要配合敌机速度
            total_steps = (0.177 * self.all_axis + 1.47) * 0.8 * total_steps
    ...
    def convert_obs_dict_to_array(self, obs_dict, if_PO):
        ...
        if self.NUM_DRONES != 1:
            for i in range(self.NUM_DRONES):
                ...
                if not if_PO:  # 不包含PO
                    if self.level == 0:
                        # 通信内容仅有obs['target_pos']前三维x2，
                        obs_array.append(np.hstack([obs['pos'], obs['rpy'], obs['vel'], obs['ang_vel'],
                                                    obs['target_pos'], obs['other_pos'], obs['last_action']]))
                    if self.level == 1 or self.level == 2:
                        # 通信内容有obs['target_pos']前三维x2，obs['other_info']速度（三维）x2或速度+欧拉角（六维）x2
                        obs_array.append(np.hstack([obs['pos'], obs['rpy'], obs['vel'], obs['ang_vel'], obs['target_pos'],
                                                    obs['other_pos'], obs['other_info'], obs['last_action']]))
        else:
            pass
    ...
    def _housekeeping(self):
        ...
        self.step_counter = 0   # 最大值应该是 8 * 1000 or 8 * limit
        ...
        if self.keep_init_pos:
            _, _, self.END_Target = self.get_init()  # 重新给出位置
            # todo 修改此处代码 修改成可以指定目标位置
            self.TARGET_POS = np.array([0, 0.5, 0.1])
        else:
    ...
    def _getDroneStateVector(self, nth_drone, with_target=False):
        ...
        if with_target:
            state_dict = {
                ...
                # self.pos[nth_drone, :]  是来自其他智能体通信的位置信息 3
            ...
            if self.level == 1:
                for i in range(self.NUM_DRONES):
                    if i != nth_drone:
                        other_inform.append(self.vel[i, :])  # np.append(, self.rpy[i, :])  合并的其他通信信息，如速度
            if self.level == 2:
                for i in range(self.NUM_DRONES):
                    if i != nth_drone:
