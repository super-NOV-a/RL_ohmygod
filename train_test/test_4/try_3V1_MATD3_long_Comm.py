...
comm_level = 1  # 1表示无人机通信为12位 位置速度通信，2表示两两通信为18位 位置速度欧拉角
Env_name = 'c3v1_1Comm'  # 新增了通信内容
Mark = 9206  # todo 测试时指定mark  位置速度通信9206  位置速度欧拉角通信9202
test_times = 300
action = 'vel'
...
if comm_level == 0:
    print('通信内容为位置信息，通信量为每帧 192bit')
elif comm_level == 1:
    if comm_level == 0:
        print('通信内容为位置与速度信息，通信量为每帧 384bit')
...
class Runner:
    def __init__(self, args):
        ...
        self.env_evaluate = C3V1_Test(gui=Env_gui, num_drones=args.N_drones, obs=ObservationType('kin_target'),
                                      ...
                                      comm_level=comm_level, all_axis=all_axis)
