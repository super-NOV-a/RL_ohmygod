...
class BaseAviary(gym.Env):
    ...
    def see_ball(self):
        for i in range(self.NUM_DRONES):
            # 为每个无人机创建一个没有碰撞体积的球体，并设置为目标位置
            collision_shape = p.createCollisionShape(p.GEOM_SPHERE, radius=0.1)
            sphere_id = p.createMultiBody(baseCollisionShapeIndex=collision_shape,
                                          basePosition=self.target_pos[i],  # 设置球体位置为无人机的目标位置
                                          baseOrientation=[0, 0, 0, 1],  # 默认方向
                                          baseVisualShapeIndex=-1)  # 不创建可视化外观
