...
def generate_non_overlapping_positions_numpy():
    # 定义空间的分割数和单元格大小
    divisions = 5
    cell_size = 2 / divisions
    # 生成所有可能的单元格坐标
    ...
    for cell_coord in cell_coordinates:
        x = np.random.uniform(cell_coord[0] * cell_size - 1, (cell_coord[0] + 1) * cell_size - 1)
        y = np.random.uniform(cell_coord[1] * cell_size - 1, (cell_coord[1] + 1) * cell_size - 1)
        z = np.random.uniform(0, 1)
        positions.append((x, y, z))
    return positions


class LyyBaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        if initial_xyzs is None:  # todo 修改初始位置
            self.cell_pos = generate_non_overlapping_positions_numpy()
        ...
        self.TARGET_POS, self.relative_pos = None, None
        if self.need_target:
            self.TARGET_POS, self.relative_pos = self.get_target()
            self.show_target()
    ...
    def get_target(self):
        """
        :return:目标位置[1,3]， 相对位置[N,3]
        """
        # 导入目标
        target_pos = np.array([[0, 0, 0.8]])
        # 记录相对位置
        if self.NUM_DRONES == 1:
            relative_pos = np.zeros((1, 3))
        else:
            relative_pos = np.zeros((self.NUM_DRONES, 3))
            angle_step = 2 * np.pi / self.NUM_DRONES
            dist_to_target = 0.5
            for i in range(self.NUM_DRONES):
                angle = i * angle_step
                relative_pos[i, 0] = dist_to_target * np.cos(angle)
                relative_pos[i, 1] = dist_to_target * np.sin(angle)
        # np.random.shuffle(relative_pos)
    ...
    def show_target(self):
        ...
        for idx in range(len(self.relative_pos)):
            self.real_pos_id = p.loadURDF("E:\PyProjects\MARL-code-pytorch\gym_pybullet_drones\\assets\\box.urdf",
                                        self.TARGET_POS[0]+self.relative_pos[idx],
                                        p.getQuaternionFromEuler([0, 0, 0]), physicsClientId=self.CLIENT,
                                        useFixedBase=True)
            p.changeDynamics(self.real_pos_id, -1, mass=0)  # 将质量设为零，即设置无重力
            p.changeDynamics(self.real_pos_id, -1, linearDamping=0, angularDamping=0)  # 设置运动类型为静态
            # 设置模型不受其他碰撞影响
            p.setCollisionFilterGroupMask(self.real_pos_id, -1, 0, 0)  # 设置碰撞组掩码
            p.setCollisionFilterPair(-1, -1, self.real_pos_id, -1, 0)  # 设置碰撞对，使模型不与其他对象发生碰撞
    ...
    def reset(self,
        ...
        ####  给定目标位置 ###########################################
        if self.need_target:
            self.TARGET_POS, self.relative_pos = self.get_target()  # target与relative 是一起存在的，每次观测都需加在一起
            self.show_target()
        #### Start video recording #################################
        self._startVideoRecording()
    ...
    def step(self, action):
        ...
        """
        # sim_start_time = time.perf_counter()    # 仿真和控制开始时间
        ...
        for STEP in range(self.PYB_STEPS_PER_CTRL):
            # step_start_time = time.perf_counter()   # 某一物理仿真 step 开始时间
            ...
            # # 输出每个物理步骤的耗时
            # print(f'物理 simulate 后耗时: {time.perf_counter() - step_start_time:.7f} 秒')  # 到次数耗时 0.0001+s
            # debug_time = time.perf_counter()
            # 睡眠以确保实时步骤持续时间
            # time_to_sleep = 1 / self.PYB_FREQ   # 剩余等待时间

            # print(f'sleep前{time.perf_counter() - debug_time:.7f} 秒')
            # while time.perf_counter() - step_start_time < time_to_sleep:
            #     pass
            # print(f'sleep后{time.perf_counter() - debug_time:.7f} 秒')
            # 输出每个物理步骤的耗时
        ...
        adjusted_rewards = [reward - 2 * p for reward, p in zip(rewards, punish)]

        # 控制步骤之间的延迟
        # control_delay = 1 / self.CTRL_FREQ
        # if control_delay > 0:
        #     while time.perf_counter() - sim_start_time < control_delay:
        #         pass
        # total_step_duration = time.perf_counter() - sim_start_time
    ...
    def _housekeeping(self):
        ...
        self.INIT_XYZS = self.get_init()  # 重新给出位置
    ...
    def assign_targets(self):
        ...
        assigned_targets = []
        remaining_targets = [self.TARGET_POS[0, :] + self.relative_pos[i, :] for i in range(num_drones)]
        chosen_targets = []

        for i in range(num_drones):
            relative_vectors = [remaining_targets[j] - self.pos[i] for j in range(len(remaining_targets))]
            distances = [np.linalg.norm(vector) for vector in relative_vectors]
            min_index = np.argmin(distances)
            min_vector = remaining_targets[min_index]
            current_target_positions = [min_vector]

            chosen_targets.append(min_vector)
            remaining_targets.pop(min_index)
            for vector in remaining_targets:
                current_target_positions.append(vector)
            if i != 0:
                for pos in chosen_targets:
                    if not any(np.array_equal(pos, ct) for ct in current_target_positions):
                        current_target_positions.append(pos)
            assigned_targets.append(current_target_positions)
    ...
    def _getDroneStateVector(self, nth_drone, with_target=False):
        ...
        if with_target:
            state_dict = {
                ...
                # 'target_pos': self.TARGET_POS[0, :] + self.relative_pos[nth_drone, :] - self.pos[nth_drone, :]  # 3
            }

            if nth_drone == 0:  # 只有第一个无人机需要计算一次
                self.assigned_targets = self.assign_targets()
            target_positions = self.assigned_targets[nth_drone] - self.pos[nth_drone, :]
            state_dict['target_pos'] = np.array(target_positions).flatten()

            other_positions = []   # 智能体指向其他智能体的向量  3*(N-1)
            for i in range(self.NUM_DRONES):
                if i != nth_drone:
                    other_positions.append(self.pos[nth_drone, :] - self.pos[i, :])
            state_dict['other_pos'] = np.array(other_positions).flatten()
            # state_dict['last_clipped_action'] = self.last_clipped_action[nth_drone, :]  # 处理后的动作
